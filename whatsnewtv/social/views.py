...
def search(request):
    ...
    return render(request, "social/search.html", { "results": results, "search_term": query, "data" : results, "query_param" : f"&query={query}"} )
...
def account_index(request):
    ...
    return render(request, "social/account_index.html", {"comments" : comments, "favorites": favorites,"include_comment_info":True})
...
def get_notifications(request):
    ...
    if request.method == 'GET':
        ...
        dump_data = [ {"from": notification.tagging_user.username, "url": reverse("details", kwargs={'type': notification.comment.moviedb.type, "id": notification.comment.moviedb.moviedb_id}), "moviedb_data":json.loads(notification.comment.moviedb.data) } for notification in notifications]
